...
$Path = "C:\f12\MonitoringLogs\O365ADAudit"
...
$Credentials =  (Get-Credential ("f12admin@"+"$Domain"+".ca"))
...
$UserListO365 = Get-MsolUser | where{$_.FirstName -ne ""} | select Firstname,Lastname,UserprincipalName,Licenses | sort licenses -Descending
...
$LicensingGroups = Get-ADGroup -SearchBase "OU=Licensing Groups,$DN" -Filter * | where{((Get-ADGroupMember -identity $_.name).name.length) -gt 0}
